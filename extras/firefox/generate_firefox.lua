...
local function hex_to_rgb(hex)
  if not hex or hex == "NONE" then return { r = 0, g = 0, b = 0 } end
...
local function generate_readme(palette_data)
  ...
  for variant_name, data in pairs(palette_data) do
    ...
    if not palettes[base_name] then palettes[base_name] = { dark = nil, light = {} } end
  ...
  for _, palette_name in ipairs(palette_names) do
    ...
    table.sort(group.light, function(a, b)
      ...
      if a_intensity and b_intensity then return tonumber(a_intensity) < tonumber(b_intensity) end
