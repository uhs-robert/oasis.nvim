...
local function generate_bat_theme(display_name, palette)
  ...
  local function add_scope(name, scope, fg, font_style)
    if not fg and not font_style then return end
  ...
  add_scope("Rust lifetime", "storage.modifier.lifetime.rust, entity.name.type.lifetime", palette.syntax.func, "italic")
  add_scope(
