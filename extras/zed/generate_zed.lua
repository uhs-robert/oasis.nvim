...
local function generate_zed_theme(name, palette)
  ...
  local active_bg = is_light and adjust_brightness(palette.bg.mantle, 0.9) or adjust_brightness(palette.bg.surface, 1.2)

  -- Generate player colors from theme accents
  local player_colors = {
    { cursor = palette.theme.primary, background = palette.theme.primary, selection = with_alpha(palette.theme.primary, "3d") },
    { cursor = palette.theme.secondary, background = palette.theme.secondary, selection = with_alpha(palette.theme.secondary, "3d") },
    { cursor = palette.theme.accent, background = palette.theme.accent, selection = with_alpha(palette.theme.accent, "3d") },
    { cursor = palette.syntax.func, background = palette.syntax.func, selection = with_alpha(palette.syntax.func, "3d") },
    { cursor = palette.syntax.string, background = palette.syntax.string, selection = with_alpha(palette.syntax.string, "3d") },
    { cursor = palette.syntax.type, background = palette.syntax.type, selection = with_alpha(palette.syntax.type, "3d") },
    { cursor = palette.syntax.constant, background = palette.syntax.constant, selection = with_alpha(palette.syntax.constant, "3d") },
    { cursor = palette.syntax.keyword, background = palette.syntax.keyword, selection = with_alpha(palette.syntax.keyword, "3d") },
  }
