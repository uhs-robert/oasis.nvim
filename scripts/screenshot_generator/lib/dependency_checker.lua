...
function DependencyChecker.check_required_commands()
  ...
  for _, cmd in ipairs(required_commands) do
    if not System.execute("which " .. cmd .. " > /dev/null 2>&1") then table.insert(missing, cmd) end
  end
...
function DependencyChecker.verify_kitty_config(kitty_conf)
  ...
  if content:match("allow_remote_control%s+yes") or content:match("allow_remote_control%s+socket%-only") then return end
