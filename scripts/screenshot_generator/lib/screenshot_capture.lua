...
function ScreenshotCapture:take_screenshot(variant, type, temp_file)
  System.execute(
    string.format("hyprshot -m window -m active -o %s -f %s-%s.png --silent 2>/dev/null", self.temp_dir, variant, type)
  )
...
function ScreenshotCapture:move_to_final_location(temp_file, final_file)
  if File.exists(final_file) then FileSystem.remove(final_file) end
