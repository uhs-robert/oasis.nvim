...
if palette_content then
  ...
  for line in palette_content:gmatch("[^\r\n]+") do
    ...
    elseif inside_colors_table then
      ...
      if line:match("^%s*}") and current_color then current_color = nil end

      if brace_depth == 0 then inside_colors_table = false end
    end
  end
end
...
for color, variants in pairs(palette_lua_colors) do
  ...
  for _, variant in ipairs(variants) do
    if not all_colors[color] or not all_colors[color][variant] then table.insert(unused, variant) end
  end
