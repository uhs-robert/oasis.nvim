...
local function normalize(c, user_overrides)
  ...
  if type(user_overrides) ~= "table" then return nil end
...
local function apply_global(resolved, user_overrides)
  for name, attrs in pairs(user_overrides) do
    if is_highlight_definition(attrs) then resolved[name] = attrs end
  end
end
...
local function apply_global_light(resolved, user_overrides, intensity)
  if user_overrides.light then merge_highlights(resolved, user_overrides.light) end

  local intensity_key = "light_" .. intensity
  if user_overrides[intensity_key] then merge_highlights(resolved, user_overrides[intensity_key]) end
end
...
local function apply_dark(resolved, palette_overrides)
  for name, attrs in pairs(palette_overrides) do
    if name ~= "light" and not name:match("^light_%d$") then resolved[name] = attrs end
  end
end
...
local function apply_light(resolved, palette_overrides, intensity)
  if palette_overrides.light then merge_highlights(resolved, palette_overrides.light) end

  local intensity_key = "light_" .. intensity
  if palette_overrides[intensity_key] then merge_highlights(resolved, palette_overrides[intensity_key]) end
end
...
local function apply_palette(resolved, user_overrides, palette_variant, is_light_mode, intensity)
  if not palette_variant then return end

  local palette_overrides = user_overrides[palette_variant]
  if type(palette_overrides) ~= "table" then return end
...
function HighlightOverrides.resolve(c, palette_name, config)
  ...
  if not user_overrides then return resolved end
  ...
  if LIGHT_MODE then apply_global_light(resolved, user_overrides, intensity) end
