...
local base = {
  ...
  theme = {
    primary_strong = p.sunset[500],
    primary = p.sundown[400],
    primary_light = p.sunshine[300],
...
local dark = {
  ...
  syntax = {
    ...
    -- Warm: (Control / Flow)
    func = opts.themed_syntax and p.khaki[500] or p.copper[200],
    builtin_func = opts.themed_syntax and p.khaki[700] or p.copper[400], -- (eg. parseInt, Array, Object etc)
...
if need_light then
  ...
  local light_theme = vim.tbl_deep_extend("force", {}, dark.theme, {
    primary_strong = p.sunrise[800],
    primary = p.sunrise[500],
    primary_light = p.sunshine[200],
  })
