...
local base = {
  ...
  theme = {
    primary_strong = p.aloe[700],
    primary = p.aloe[700],
    primary_light = p.aloe[500],
...
local dark = {
  ...
  syntax = {
    -- Cold: (Data)
    parameter = opts.themed_syntax and p.gold[400] or p.palm[500],
    preproc = opts.themed_syntax and p.khaki[500] or p.olive[500], -- (imports)
    macro = opts.themed_syntax and p.khaki[700] or p.olive[600], -- (imports)
    ...
    -- Warm: (Control / Flow)
    func = p.copper[300],
    builtin_func = p.copper[500], -- (eg. parseInt, Array, Object etc)
  ...
  ui = {
    ...
    match_parent = { bg = p.stone[900], fg = base.palette.secondary[500] },
    dir = p.sky[500],
...
if need_light then
  ...
  local light_ui = vim.tbl_deep_extend("force", {}, dark.ui, {
    ...
    match_parent = { bg = dark.ui.match_parent.bg, fg = p.lavender[800] },
  })
