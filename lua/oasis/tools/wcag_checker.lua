...
function WcagChecker.analyze_palette(palette_name)
  ...
  local function check(label, fg, bg, large_text)
    ...
    if not fg or not bg or fg == "none" or bg == "none" then return end
  ...
  -- Syntax highlighting (Transitional colors)
  ...
  check("Macro (macro on bg.core)", palette.syntax.macro, palette.bg.core)
  check("Builtin Constants (builtin_const on bg.core)", palette.syntax.builtin_const, palette.bg.core)
  check("Builtin Vars (builtin_var on bg.core)", palette.syntax.builtin_var, palette.bg.core)

  -- Syntax highlighting (Cold colors)
  ...
  -- Syntax highlighting (Warm colors)
  check("Functions (func on bg.core)", palette.syntax.func, palette.bg.core)
  check("Builtin Functions (builtin_func on bg.core)", palette.syntax.builtin_func, palette.bg.core)
...
function WcagChecker.print_comparison_table(all_results)
  ...
  for _, results in ipairs(all_results) do
    if not results.error then table.insert(sorted_results, results) end
  end
