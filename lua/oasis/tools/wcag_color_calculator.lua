...
function Color.from_hex(hex)
  if not hex or type(hex) ~= "string" then return Color.new(0, 0, 0) end
  hex = hex:gsub("#", "")
...
function Color:to_hsl()
  ...
  if delta == 0 then return 0, 0, lightness end
...
function Color.from_hsl(hue, saturation, lightness)
  if saturation == 0 then return Color.new(lightness, lightness, lightness) end
...
function WcagCalculator.contrast_ratio(color1, color2)
  if type(color1) == "string" then color1 = Color.from_hex(color1) end
  if type(color2) == "string" then color2 = Color.from_hex(color2) end
...
function WcagCalculator.get_compliance_level(ratio, large_text)
  ...
  if ratio >= threshold then return "AAA" end
  threshold = large_text and STANDARDS.AA_LARGE or STANDARDS.AA_NORMAL
  if ratio >= threshold then return "AA" end
  return "Fail"
end
...
function WcagCalculator.adjust_for_target(hex_color, background_hex, target_ratio, max_iterations)
  ...
  for _ = 1, max_iterations do
    ...
    if math.abs(max_l - min_l) < 0.001 then break end
    ...
    if math.abs(test_ratio - target_ratio) < 0.01 then return test_hex, test_ratio end
...
function WcagCalculator.extract_colors_from_palette(palette)
  ...
  if palette.syntax then
    for key, value in pairs(palette.syntax) do
      if type(value) == "string" and value:match("^#") then colors["syntax." .. key] = value end
    end
  end
  ...
  if palette.theme then
    for key, value in pairs(palette.theme) do
      if type(value) == "string" and value:match("^#") then colors["theme." .. key] = value end
    end
  end
  ...
  if palette.fg then
    for key, value in pairs(palette.fg) do
      if type(value) == "string" and value:match("^#") then colors["fg." .. key] = value end
    end
  end
  ...
  if palette.ui then
    for key, value in pairs(palette.ui) do
      if type(value) == "string" and value:match("^#") then colors["ui." .. key] = value end
    end
  end
  ...
  if palette.terminal then
    for key, value in pairs(palette.terminal) do
      if type(value) == "string" and value:match("^#") then colors["terminal." .. key] = value end
    end
  end
...
function WcagCalculator.analyze_palette(palette_name, target_ratio, custom_targets, mode)
  ...
  if next(colors) == nil then return nil, "No colors found in palette" end
